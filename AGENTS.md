...
10. **Cheat sheet.** A table a tired person can screenshot. Always ends with a **Use / skip** remark: when to reach for *this* method, when to skip it, what it pays you, what it costs you. Short. Honest. Points at siblings instead of hand-waving “use something else.” Prefer **bullets** under Reach / Skip. Do **not** add a reach/skip SVG that only repeats those bullets. A drawing there has to show something the text cannot (weeks vs a cloud). Optional: **Also called** (in a room) as a small `here | there` table — house word → the word you’d say in a room. Only for words *this* note introduced. Do not clone a glossary into every sequel. Do not dump it as one dotted sentence.
